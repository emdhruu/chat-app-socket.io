...
export default class UserController {
  ...
  sendFriendRequest = async (
    ...
    try {
      ...
      const sender: any = await User.findById(myId).select(
        "fullName  profilePic"
      );
      const recipientSocketId = getReceiverSocketId(friendId);
      if (recipientSocketId) {
        return io.to(recipientSocketId).emit("new-friend-request", {
          from: myId,
  ...
  acceptFriendRequest = async (
    ...
    try {
      ...
      if (senderSocketId) {
        return io
          .to(senderSocketId)
          .emit("friend-request-accepted", { by: myId });
      }
      if (recipientSocketId) {
        return io
          .to(recipientSocketId)
          .emit("friend-request-accepted", { by: friendId });
      }
  ...
  rejectFriendRequest = async (
    ...
    try {
      ...
      if (recipientSocketId) {
        io.to(recipientSocketId).emit("friendRequestRejected", {
          by: friendId,
        });
      }
